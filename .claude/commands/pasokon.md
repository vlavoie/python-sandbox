...
- `_send_execute` (chat handler) uses `show_progress="full", show_progress_on=self.review_input` — this targets Gradio's native loading overlay to the input box only, leaving the chatbot unaffected while streaming. `show_progress_on` is a Gradio 5 parameter that restricts which output components get the overlay. This is the EVENT KWARG mechanism, NOT `gr.Progress()` (the function parameter). They are completely separate. `show_progress="full"` + `show_progress_on` is safe with `review_chatbot` in outputs; `gr.Progress()` is banned.
