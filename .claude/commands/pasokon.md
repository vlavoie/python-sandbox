...
- `_send_execute` (chat handler) uses `show_progress="full", show_progress_on=self.review_input` — Gradio 5 param that targets the loading overlay to the input only. CRITICAL: `review_input` must NOT be in `_send_execute`'s `outputs` list. Any component in `outputs` that receives a yield (even `gr.update()` no-op) gets its loading overlay cleared on that yield. Keeping `review_input` out of outputs means the overlay persists for the full streaming duration. `review_input` stays in `inputs` (to read the message) and in `_send_start`/`_send_finish` outputs (for lock/unlock). This is the EVENT KWARG mechanism, NOT `gr.Progress()` (the function parameter) — `gr.Progress()` is banned.
